...
class PlayViewController: ViewController, HasContext {
  ...
  @objc func playGameButtonTapped(button _: UIButton) {
  ...
  override func loadView() {
    ...
    secondsBetweenTurnsButton = UIButton.button(
      title: Lets.secondsBetweenTurnsString,
      target: self,
      selector: #selector(secondsBetweenTurnsButtonTapped),
      font: Theme.Fonts.playLabels
    )
    let secondsBetweenTurnsStackView = StackView(
      arrangedSubviews: [secondsBetweenTurnsButton],
      axis: .horizontal,
      spacing: 8,
      distribution: .fillEqually
    )

    squareButton = UIButton.button(
      title: Lets.squaresString,
      target: self,
      selector: #selector(squareButtonTapped),
      font: Theme.Fonts.playLabels
    )
    squareButton.titleLabel?.numberOfLines = 2
    squareButton.titleLabel?.textAlignment = .center
    numberButton = UIButton.button(
      title: Lets.numbersString,
      target: self,
    ...
    colorButton = UIButton.button(
      title: Lets.colorsString,
      target: self,
      selector: #selector(colorButtonTapped),
      font: Theme.Fonts.playLabels
    )
    colorButton.titleLabel?.numberOfLines = 2
    colorButton.titleLabel?.textAlignment = .center

    levelButton = UIButton.button(
      title: Lets.levelString,
      target: self,
      selector: #selector(levelButtonTapped),
      font: Theme.Fonts.playLabels
    )
    turnsButton = UIButton.button(
      title: Lets.turnsString,
      target: self,
      selector: #selector(turnsButtonTapped),
      font: Theme.Fonts.playLabels
    )
    let levelAndTurnsStackView = StackView(
      arrangedSubviews: [levelButton, turnsButton],
      axis: .horizontal,
      spacing: 8,
      distribution: .fillEqually
    )
    ...
    rowsButton = UIButton.button(
      title: Lets.rowsString,
      target: self,
      selector: #selector(rowsButtonTapped),
      font: Theme.Fonts.playLabels
    )
    rowsButton.isEnabled = squaresIsOn
    columnsButton = UIButton.button(
      title: Lets.columnsString,
      target: self,
      selector: #selector(columnsButtonTapped),
      font: Theme.Fonts.playLabels
    )
    columnsButton.isEnabled = squaresIsOn
    let rowsAndColumnsStackView = StackView(
      arrangedSubviews: [rowsButton, columnsButton],
      axis: .horizontal,
      spacing: 8,
      distribution: .fillEqually
    )

    playGameButton = UIButton.button(
      title: Lets.playl10n,
      target: self,
      selector: nil,
      font: Theme.Fonts.playLabels
    )
    playGameButton.addTarget(
      self,
      action: #selector(playGameButtonTapped(button:)),
      for: UIControl.Event.touchUpInside
    )
    playGameButton.backgroundColor = Theme.Colors.playButtonBackground
    playGameButton.setTitleColor(
      Theme.Colors.playButtonFont,
      for: UIControl.State()
    )
    playGameButton.layer.cornerRadius = 5
    let playGameButtonStackView = StackView(
      arrangedSubviews: [playGameButton],
      axis: .horizontal,
      spacing: 0,
      distribution: .fillEqually
    )
    ...
    if UIScreen.main.bounds.height > 0 {
      views = [
        secondsBetweenTurnsStackView, squareButton, numberButton, colorButton,
      ...
      views = [
        secondsBetweenTurnsStackView, typeButtonsStackView,
        levelAndTurnsStackView, rowsAndColumnsStackView,
        playGameButtonStackView,
      ]
    }

    let preStackView = StackView(
    ...
    let stackView = StackView(
      arrangedSubviews: [lastGameLabel, preStackView],
      axis: .vertical,
      spacing: 8,
      distribution: .fill
    )
...
//    )
  ...
  func typeButtonTapped(type: GameType, button _: UIButton) {

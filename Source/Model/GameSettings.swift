...
struct GameSettings {
  ...
  var squareHighlightTime: Double {
    get {
      return defaults.object(forKey: Lets.squareHighlightTimeKey) as? Double
        ?? 1
    }
  ...
  var secondsBetweenTurns: Double {
    get {
      return defaults.object(forKey: Lets.secondsBetweenTurnsKey) as? Double
        ?? 3
    }
  ...
  var types: [GameType] {
    get {
      return (defaults.object(forKey: Lets.typesKey) as? [Int] ?? [0, 1, 2]).map
      { GameType.from(value: $0) }
    }

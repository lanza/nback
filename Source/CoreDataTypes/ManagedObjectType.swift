...
extension ManagedObjectType where Self: ManagedObject {
  ...
  public static func findOrFetch(
    ...
    guard
      let object = materializeObject(in: context, matchingPredicate: predicate)
    else {
  ...
  public static func materializeObject(
    in context: NSManagedObjectContext,
    matchingPredicate predicate: NSPredicate
  ) -> Self? {
    for object in context.registeredObjects where !object.isFault {
      guard let res = object as? Self, predicate.evaluate(with: res) else {
        continue
      }
      return res
    }

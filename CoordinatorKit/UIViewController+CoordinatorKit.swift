...
//MARK: - Swizzling
fileprivate let swizzling: (UIViewController.Type) -> Void = { viewController in

  let swizzle: (Selector, Selector) -> Void = { first, second in
...
extension UIViewController {
  ...
  public var coordinator: Coordinator! {
    get {
      return objc_getAssociatedObject(self, &AssociatedKeys.coordinator)
        as? Coordinator
    }
    set {
